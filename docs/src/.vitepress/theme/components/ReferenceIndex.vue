...
const props = defineProps({
  data: {
    type: Array as PropType<APIGroup[]>,
    default: () => [],
  },
})
    ...
    .map(section => {
      ...
      const matchedGroups = section.items
        .map(item => {
          ...
          // filter headers
          const matchedHeaders = item.indexes.filter(
            ({ text, link }) => matches(text) || matches(link),
          )
          return matchedHeaders.length
            ? { text: item.text, link: item.link, indexes: matchedHeaders }
            : null
        })
        .filter(i => i)
    ...
    <div v-for="section of filtered" :key="section.text" class="api-section">
      ...
      <div class="api-groups">
        <div v-for="item of section.items" :key="item.text" class="api-group">
          <h3>{{ item.text }}</h3>
          <ul>
            <li v-for="h of item.indexes" :key="h.link">
